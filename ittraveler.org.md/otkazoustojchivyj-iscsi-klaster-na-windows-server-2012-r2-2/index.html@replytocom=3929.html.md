...
На первом сервере включаем компонент &laquo;отказоустойчивая кластеризация&raquo;
...
Выбираем &laquo;Настроить роль&raquo;
...
Далее выбираем пункт &#8212; &laquo;Новая цель ISCSI&raquo;
...
Далее нажимаем &laquo;Создать&raquo; и ожидаем окончания работы мастера.
Готово! Теперь указанные клиенты могут подключаться к кластеру ISCSI. Для подключения нужно использовать dns имя кластера.
Удачной установки!
&nbsp;
Related posts:Ошибка Database Mirroring login attempt by user ‘Domain\user.' failed with error: ‘Connection handsh...Восстановление объектов Active Directory: сборник сценариевСоздание индивидуальных адресных книг в Office 365 и Exchange online
