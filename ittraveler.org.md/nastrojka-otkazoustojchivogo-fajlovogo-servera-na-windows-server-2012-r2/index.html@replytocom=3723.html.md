...
  Первым делом убедитесь что сервер введен в домен Active DirecTORy, далее установите роли DFS и файлового сервера
Выберите следующие роли и установите их.
Далее создайте структуру папок на отдельном диске.
Теперь включим общий доступ.
Выберите &laquo;расширенная настройка&raquo;
Далее выберите &laquo;Разрешения&raquo; и установите права как на скриншоте.
...
Теперь нажмите кнопку &laquo;Дополнительно&raquo;
...
(adsbygoogle = window.adsbygoogle || []).push({});
Далее указываем путь к папке. Путь можно посмотреть тут, выбрав &laquo;open share&raquo;.
Создаем папку.
...
Должно получится так
Теперь включим ABE.
В диспетчере сервера откройте &laquo;Файловые службы&raquo; &#8212; &laquo;Общие ресурсы&raquo;.
Выберите каталог IT, и нажмите свойства.
Далее выбираем параметры, и включаем функцию &#8212; &laquo;Перечисление на основе доступа&raquo;.
...
Выбираем пространство имен, которое хотим реплицировать, и выбираем пункт &laquo;Добавить конечный объект папки&raquo;.
...
Должно получится так
Теперь добавьте 2-й в &laquo;серверы пространства имен&raquo; на основном сервере.
...
&nbsp;
&nbsp;
Related posts:Создание индивидуальных адресных книг в Office 365 и Exchange onlineОшибка ERR: CERT_COMMON_NAME_INVALID в Chrome при использовании SSL сертификата.Автоматический аудит компьютеров в Active Directory через powershell.
